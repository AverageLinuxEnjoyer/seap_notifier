...
impl Scraper {
    pub fn new(reqwest_client: Client) -> Self {
        Self { reqwest_client }
    }

    pub async fn get_last_adv_ids(&self, n: usize, time: OffsetDateTime) -> Result<Vec<u32>> {
        let resp = self
            .reqwest_client
    ...
    pub async fn get_adv(&self, id: u32) -> Result<Advertisment> {
        let resp = self
            .reqwest_client
            .get(format!(
